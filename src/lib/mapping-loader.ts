...
import { type MappingConfig, mappingConfigSchema } from "../schemas/configSchema.js";
 ...
 */
export function loadMappingConfig(filePath: string): MappingConfig {
  ...
  try {
    ...
    throw new Error(
      `Error reading mapping configuration file at ${filePath}: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Invalid JSON in mapping configuration file at ${filePath}: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
 ...
 */
export function loadAndMergeMappingConfigs(
