 ...
 */
function extractBootstrapVariables(
  cssContent: string,
  prefix = "--bs-",
): Record<string, { value: string; themeName: string }> {
  const variables: Record<string, { value: string; themeName: string }> = {};
  ...
  if (matches) {
    for (const match of matches) {
      ...
      const namePartRegex = new RegExp(
        `(--(${prefix.substring(2)}[a-z0-9-]+))\\s*:\\s*([^;]+)\\s*;`,
      );
...
export async function runExtraction(options: CliOptions): Promise<void> {
  ...
  try {
    ...
    const colorVars = Object.keys(extractedVariables).filter(
      (v) =>
        v.includes("-color") ||
        v.includes("-bg") ||
    ...
    const spacingVars = Object.keys(extractedVariables).filter(
      (v) =>
        v.includes("-spacing") ||
        v.includes("-padding") ||
        v.includes("-margin") ||
