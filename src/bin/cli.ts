...
import { TransformMethod, type TransformResult, transformCss } from "../lib/transformer.js";
...
export function parseArguments(argv: string[]): CliOptions {
  ...
  program
    ...
    .requiredOption(
      "-c, --config <configFile>",
      "Path to the JSON configuration file for default variable mapping",
    )
    .option(
      "--overrides <overridesFile>",
      "Path to the JSON configuration file with variable overrides",
    )
    .option(
      "-m, --method <method>",
      "Transformation method to use (ast or regex, default: ast)",
      "ast",
    )
...
export async function runTransformation(options: CliOptions): Promise<void> {
  ...
  try {
    ...
    const { css: transformedCss, targetVariables } = transformCss(
      cssContent,
      config,
      options.method,
    );
